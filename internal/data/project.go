...
func ValidateProjectInputSemantic(v *validator.Validator, p *ProjectInput, u UserModel, r RoleModel) {
	if p.Feature != nil {
	...
	if len(p.Assignments) > 0 {
		for _, assignment := range p.Assignments {
			v.Check(assignment.EmployeeID != nil, "assignments_employee_id", "must be provided")
			v.Check(assignment.RoleID != nil, "assignments_role_id", "must be provided")
			_, err := u.GetById(*assignment.EmployeeID)
			v.Check(err == nil, "assignments_employee_id", "must be registered user")
			_, err = r.Get(*assignment.RoleID)
			v.Check(err == nil, "assignments_role_id", "must be registered role")
		}
	}
}
...
func (m ProjectModel) Update(project *ProjectRequest) error {
	query := `
		UPDATE project
		SET project_id = $1, proposal_id = $2, name = $3, status = $4, feature = $5, images = $6, note = $7, version = version + 1, updated_at = $8
		WHERE internal_id = $9 AND version = $10
...
func (m ProjectModel) GetAll(qs ProjectQsInput, bbox BoundingBox) ([]*ProjectResponse, Metadata, error) {
	query := fmt.Sprintf(`
		SELECT count(*) OVER(), 
			...
			COALESCE(pc.clients, ARRAY[]::jsonb[]) AS clients, 
			COALESCE(pa.assignments, ARRAY[]::jsonb[]) AS assignments,
			p.images, p.note, p.version, p.created_at, p.updated_at
		FROM project p
		LEFT JOIN (
			SELECT 
				pc.project_internal_id,
