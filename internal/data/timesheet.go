...
// func ValidateTimesheetInputSemantic(v *validator.Validator, t *TimesheetInput) {
...
// 	v.Check(len(*t.Description) <= 300, "description", "cannot exceed 300 bytes long")
// 	// }
// }
...
// func (m TimesheetModel) Get(id string) (*Timesheet, error) {
	...
	query := fmt.Sprintf(`
		...
		WHERE (
			...
			( 
				(
					t.work_date BETWEEN 
					CAST(NULLIF($10, '') AS DATE) 
					AND 
					CAST(NULLIF($11, '') AS DATE)
				)
				OR 
				($10 = '' OR $11 = '') 
			)
