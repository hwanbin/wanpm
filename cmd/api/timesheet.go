...
func (app *application) createTimesheetHandler(w http.ResponseWriter, r *http.Request) {
	...
	timesheets, err := app.models.Timesheet.InternalGetAll(timesheetRequest.UserID, timesheetRequest.WorkDate)
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}
	if data.ValidateTotalMinutes(v, timesheets, timesheetRequest.WorkMinutes); !v.Valid() {
		app.failedValidationResponse(w, r, v.Errors)
		return
	}
	...
	err = app.writeJSON(w, http.StatusCreated, envelope{"timesheet": timesheetRequest}, nil)
	if err != nil {
		app.serverErrorResponse(w, r, err)
	}
}
...
func (app *application) listTimesheetHandler(w http.ResponseWriter, r *http.Request) {
	...
	if data.ValidateTimesheetQueryString(v, input); !v.Valid() {
		app.failedValidationResponse(w, r, v.Errors)
		return
	}
